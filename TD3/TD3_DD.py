...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        return self.max_action * torch.softmax(self.l3(a), dim=-1)  # 通过tanh后再进行尺寸缩放成实际Action值
...
class TD3(object):
    ...
    def select_action(self, state):
        ...
        # 加入噪声
        # action_probs = action_probs + (torch.normal(0., 0.001, size=(1, 2)).to(device)).clip(-0.05, 0.05)  # 加入噪声
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with torch.no_grad():
            # 添加clipped噪声
            noise = (
                    torch.normal(0., 0.001, size=(batch_size, 2)).to(device)  # 加入噪声
            ).clamp(-0.05, 0.05)

            # next_action = torch.softmax(self.actor_target(next_state) + noise, dim=-1)
...
def main():
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        for t in range(max_timesteps):
            ...
            action, action_probs = policy.select_action(state)
            ...
            # 保存数据
            replay_buffer.add(state, action_prob, next_state, reward, done_bool)
